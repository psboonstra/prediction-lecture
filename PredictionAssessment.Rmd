...
observed_results <-
  ...
  filter(model_name %in% c("(truth)","null") | step != "testing" | mspe_ranking == min(mspe_ranking)) %>%
  ungroup();
```
...
change_in_optimism <- 
  ...
  group_by(sim) %>%
  filter(model_name %in% c("(truth)","null") | delta == "validation_minus_training" | mspe_ranking == min(mspe_ranking)) %>%
  ungroup() %>%
...
predict_models %>%
  ...
  summarize(mspe = mean((y - value)^2),
            ...
            zero_one = mean((1-y)*1*(value >= 0.5) + y*1*(value < 0.5)),
