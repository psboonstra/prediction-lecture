---
title: "Assessing Prediction Models"
...
- Forward selection with max of 1 variable / 20 observations in training data ('forward_pruned')
...
forward_pruned_model <- 
  stepAIC(null_model,
          scope = list(upper = full_fmla),
          direction = "forward",
          steps = max(1, floor(training_subset / 20)),
          trace = F);
...
- $n = 100;100;100$ observations in training;validation;testing
- Same generating model as previously but repeat 2000 times
...
```{r, fig.height = 1.4*fig.y, fig.width = fig.x}
...
```{r, fig.height=0.72*fig.x, fig.width=1.05*fig.x}
roc_plot <- 
...
- For fixed specificity of 0.8, can plausibly expect sensitivity of about 0.3

- If we classify based upon cutoff of $\hat Y(x) > 0.5$ versus $\hat Y(x) \leq 0.5$,we would expect sensitivity ~0.6 and specificity ~0.57
...
```{r, echo = FALSE, fig.height=0.72*fig.x, fig.width=0.72*fig.x}
